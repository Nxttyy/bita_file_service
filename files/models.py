...
from django.core.files.base import File  # Fix for 'File' not defined
...
def file_upload_to(instance, filename):
    ...
    if hasattr(instance, 'optimized_image') and instance.optimized_image:
        return os.path.join(file_folder, f"{instance.stored_as}_optimized.{file_extension}")
    elif hasattr(instance, 'thumbnail') and instance.thumbnail:
        return os.path.join(file_folder, f"{instance.stored_as}_thumbnail.{file_extension}")
    else:
...
class FileModel(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.file_extension in ['.png', '.jpg', '.jpeg', '.gif']:
            ...
            super().save(update_fields=['optimized_image', 'thumbnail'])  # Save only these fields
